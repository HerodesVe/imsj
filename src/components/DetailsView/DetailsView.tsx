    ...
    <div className={styles.detailsContainer}>
      ...
      <div className={styles.form}>
        ...
        <div style={{ display: "flex", gap: "20px", width: "80%" }}>
          ...
          <InputField label="Fecha de vigencia" value={data.infoEstado?.fechaVigencia || ""} />
        </div>
        <div style={{ display: "flex", gap: "20px", width: "70%" }}>
          <InputField label="Certificado desde" value={data.fechaCertificacionDesde || ""} />
          <InputField label="Certificado hasta" value={data.fechaCertificacionHasta || ""} />
          <InputField label="Fecha de actuación" value={data.fechaActualizacion || ""} />
        ...
        <div style={{ display: "flex", gap: "20px", width: "60%" }}>
          ...
          <InputField label="Fecha egreso int." value={data.infoInteracion?.fechaEgresoInternacion || ""} />
          <InputField label="Patología" value={data.infoPatologia?.patologia || ""} />
        </div>
        <div style={{ display: "flex", gap: "20px", width: "70%" }}>
          <RadioInput
            options={[
              { label: "Es Excepción", checked: data.infoPatologia?.esExcepcion || false },
              { label: "Internación", checked: data.infoInteracion?.esInternacion || false },
            ]}
          />
        </div>

        <div style={{ display: "flex", gap: "20px", width: "40%" }}>
          <InputField label="Fecha de Reintegro" value={data.infoReintegroAnticipado?.fechaReintegro || ""} />
          <RadioInput options={[{ label: "Reintegro anticipado", checked: data.infoReintegroAnticipado?.esReintegroAnticipado || false }]} />
        </div>
      </div>
